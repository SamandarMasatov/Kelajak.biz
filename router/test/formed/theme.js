...
const THEME = require('../../../models/test/formed/theme');
...
router.get('/all', async (req, res, next) => {
    ...
    const result = await THEME.find()
        ...
        .populate({ path: "form_ID", select: "name" })
        .sort({ date: -1 })
    const subject = await Subject.find().sort({ date: -1 })
    res.render("./FREE_test/formed/theme", { title: "Kelejak.uz", layout: "./layout", user, result, subject });
});
